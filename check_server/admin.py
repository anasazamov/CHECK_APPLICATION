...
class IsActiveFilterApp(IsActiveFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == 'yes':
            ...
            active_ids = [obj.id for obj in queryset if is_port_open(obj.server.ipv4, obj.port)]
            return queryset.filter(id__in=active_ids)
        elif self.value() == 'no':
            inactive_ids = [obj.id for obj in queryset if not is_port_open(obj.server.ipv4, obj.port)]
            return queryset.filter(id__in=inactive_ids)
        return queryset

@admin.register(Server) 
class ServerAdminView(admin.ModelAdmin):
    list_display = ("id", "name", "ipv4", "is_active", "username", "password", 'company')
...
class ApplicationAdminView(admin.ModelAdmin):
    ...
    list_display = ["id", "name_run_on_server", "ipv4", "port", "server_name", "username", "password", "is_active", 'company']
...
class DomainAdminView(admin.ModelAdmin):
    list_display = ['id', 'domain', 'server', 'username', "password", 'is_active', "to_expire", "valid_to", 'company']
